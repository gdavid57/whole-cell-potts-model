...
class GrowthSteppableThreaded(SteppableBasePy):
    ...
    def process_single_cell(self, cell):
        """Traiter une cellule unique dans un thread"""
        try:
            ...
            new_target_volume = self.spherocylinder_volume(
                new_major_length, self.minor_length
            )

            should_divide = new_major_length > 2 * self.initial_major_length

            return {
                "cell": cell,
                "new_major_length": new_major_length,
            ...
            return {"cell": cell, "success": False, "error": str(e)}
    ...
    def step(self, mcs: int):
        ...
        # Initialiser les cellules nouvellement divisées
        print("Init cells which had divided")
        with ThreadPoolExecutor() as executor:
            executor.map(
                self.initialize_cell,
                [cell for cell in self.cell_list if cell.dict["has_divided"]],
            )
        ...
        if mcs % self.cc3d_steps == 0:
            ...
            with ThreadPoolExecutor() as executor:
                print("Process cells")
                results = executor.map(self.process_single_cell, self.cell_list)
                ...
                for result in results:
                    if result["success"]:
                        ...
                        # Note: Ces opérations doivent être thread-safe
                        self.bacteria_model_plugin.setMajorAxisLength(
                            cell, result["new_major_length"]
                        )
            ...
            # Marquer les cellules à diviser
            if cells_to_divide:
